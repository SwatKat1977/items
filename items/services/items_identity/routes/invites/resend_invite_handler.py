...
from items.services.items_identity.routes.invites.schemas import (
    SCHEMA_RESEND_INVITE_REQUEST)
from items.services.items_identity.services.invite_management_service import (
    InviteManagementService, InviteResendStatus)


class ResendInviteHandler(BaseApiRoute):
    """Handles POST /invites/resend — refresh token and expiry on a pending invite."""
    ...
    @validate_json(SCHEMA_RESEND_INVITE_REQUEST)
    async def resend_invite(self, request_msg: ApiResponse) -> Response:
        ...
        result = await self._service.resend_invite(email_address)

        if result.status == InviteResendStatus.NO_PENDING_INVITE:
            return Response(
