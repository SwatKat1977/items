...
class TestService(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.service = Service(self.mock_quart_instance)

        # Mock the logger
        self.mock_logger_instance = MagicMock()
        self.service._logger = self.mock_logger_instance

        # Mock Configuration
        self.mock_config_instance = MagicMock()
        patch("service.Configuration", return_value=self.mock_config_instance).start()
        self.addCleanup(patch.stopall)

    @patch("service.Configuration")
    @patch("service.__version__", new="V1.0.0-123-alpha")
    async def test_initialise_success(self, mock_configuration_class):
        ...
        self.service._manage_configuration = MagicMock(return_value=True)
        self.service._open_database = MagicMock(return_value=True)

        # Patch blueprint creation if they involve side effects
        with patch("service.create_api_routes") as mock_create_routes:
    ...
    async def test_initialise_failure_configuration(self):
        ...
        self.service._manage_configuration = MagicMock(return_value=False)
        ...
        self.service._manage_configuration.assert_called_once()
    ...
    @patch.dict(os.environ, {"ITEMS_CMS_SVC_CONFIG_FILE_REQUIRED": "1"})
    def test_manage_configuration_success(self, mock_isfile):
    ...
    @patch.dict(os.environ, {"ITEMS_CMS_SVC_CONFIG_FILE": "config_file_path"})
    @patch.dict(os.environ, {"ITEMS_CMS_SVC_CONFIG_FILE_REQUIRED": "1"})
    def test_manage_configuration_process_config_file_not_found(self):
        """Test _manage_configuration when Configuration.process_config throws ValueError."""
        self.mock_config_instance.configure = MagicMock()
        self.mock_config_instance.process_config = MagicMock()
        self.mock_config_instance.logging_log_level = "DEBUG"
        ...
        self.assertFalse(result,
                         "_manage_configuration should fail when the config file doesn't exist")
        self.mock_config_instance.configure.assert_called_once_with(CONFIGURATION_LAYOUT,
                                                                    "config_file_path",
                                                                    True)
    ...
    async def test_main_loop_execution(self):
        ...
        try:
            await asyncio.wait_for(self.service._main_loop(), timeout=1.0)
